...
public class ProductDao {
	...
	public void addProduct(Product p) throws SQLException {
		...
		String sql = "insert into products(name, price, description, sold, category, remain, imgurl) values(?,?,?,?,?,?,?)";
		QueryRunner runner = new QueryRunner(DataSourceUtils.getDataSource());
		runner.update(sql, p.getName(), p.getPrice(), p.getDescription(),
				p.getSold(),p.getCategory(), p.getRemain(), p.getImgurl());
	}
	// 查找所有商品
	...
	// 生成订单时，将商品数量减少，出售数量增加
	public void changeProductNum(Order order) throws SQLException {
		String sql = "update products set remain=remain-?, sold=sold+? where id=?";
		...
		for (int i = 0; i < params.length; i++) {
			...
			params[i][1] = items.get(i).getBuynum();
	...
	public void editProduct(Product p) throws SQLException {
		...
		String sql  = "update products " +
				      "set  name=?,price=?,category=?,sold=?,remain=?,description=? ";
		//判断是否有图片
...
//		Object obj = new Object[] { (currentPage - 1) * currentCount, currentCount };
	...
	//返回<类别，销售总额>
	public HashMap<String, Double> getCateStatistic() throws SQLException {
		// TODO Auto-generated method stub
		String sql = "SELECT category, sum(price*sold) FROM products GROUP BY category;";
		QueryRunner runner = new QueryRunner(DataSourceUtils.getDataSource());
		//每一行都封装为一个Bean，所有Bean放到result中
		List<Object[]> result=  runner.query(sql, new ArrayListHandler());
		HashMap<String, Double> cateList = new HashMap<>();
		//集合的遍历
		for(Object[] objs : result){
			cateList.put(objs[0].toString(), Double.parseDouble(objs[1].toString()));
		}
		return cateList;
	}
	
	//返回<天数，销售总额>
		public HashMap<String, Double> getDayStatistic() throws SQLException {
			// TODO Auto-generated method stub
			String sql = "SELECT DAY(orders.ordertime),sum(money) FROM orders WHERE ordertime between (SELECT DATE_ADD(now(),INTERVAL -1 MONTH)) and now() GROUP BY DAY(orders.ordertime) order by DAY(orders.ordertime);";
			QueryRunner runner = new QueryRunner(DataSourceUtils.getDataSource());
			//每一行都封装为一个Bean，所有Bean放到result中
			List<Object[]> result=  runner.query(sql, new ArrayListHandler());
			HashMap<String, Double> cateList = new HashMap<>();
